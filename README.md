...
- 시맨틱 태그 사용 및 이미지 메타태그(alt) 관리를 통해 `Lighthouse` 검사 기준 접근성과 SEO 점수 **100점**을 달성했습니다.
...
- 헤더·모달·버튼 등 레이어 간 충돌을 방지하기 위해 용도별 `z-index` 값을 CSS 변수로 분리하여 전역 스타일에서 일괄 관리, 컴포넌트별 하드코딩을 제거했습니다.
